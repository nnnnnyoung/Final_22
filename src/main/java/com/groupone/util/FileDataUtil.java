...
public class FileDataUtil {
   ...
   }; // 현재 코드는 활용하지는 않는다. 얘는 선언이지 기능이 동작하지는 않는다. 
   ...
   @ResponseBody 
   //어떤 데이터를 포함하여 전송. 어노테이션 view 지정하지 않고 바로 클라이언트 요청으로 응답.
   ...
   public String[] fileUpload(MultipartFile [] file) throws IOException {
	   ...
	   for(int i=0; i<file.length; i++) {
		   if(file[i].getOriginalFilename()!="") {//실제 file 객체가 존재한다면..(첨부파일을 올리지 않을 경우도 있음)
			   ...
			   //String[] files = new String[] {saveName};// 형변환 file[0] 파일명이 들어간다. 
			   byte[] fileData = file[i].getBytes(); //큰데이터를 한번에 전송하지 않고  byte단위로 쪼개서 전송함
