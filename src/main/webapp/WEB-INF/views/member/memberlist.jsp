	...
	<!-- 				<li class="nav-item"><a href="pricing" class="nav-link">기간대별</a></li> -->
					...
					<li class="nav-item active" ><c:choose>
							<c:when test="${login_id!=null}">
								<a href="<%=request.getContextPath()%>/myInfo?id=${login_id}" class="nav-link" style="margin-right:-15px;">${login_id}님</a>
								<li class="nav-link" style="margin-top:7px;"><a href="logout" style="color:#01d28e;">로그아웃</a></li>
							</c:when>
							<c:otherwise>
				...
				<div class="col-md-9 ftco-animate pb-5">
					<p class="breadcrumbs">
						<span class="mr-2"><a href="index.html">Home <i
								class="ion-ios-arrow-forward"></i></a></span> <span>예약정보 <i
							class="ion-ios-arrow-forward"></i></span>
					</p>
					<h1 class="mb-3 bread">게시판리스트</h1>
				</div>
			</div>
		</div>
	</section>
		...
		<div boader="1" id="t">
			<table>
				...
				<c:forEach items="${list}" var="cvo">
					<tr class='record'>
						<td><a href="boardone?bno=${cvo.bno }&id=${login_id}">${cvo.bno}</a></td>
						<td>${cvo.title}</td>
						<td>${cvo.bindate }</td>
					</tr>
				</c:forEach>
			...
			<div class="block-27">
				<ul>
					...
					<c:if test="${pageVO.prev}">
						<a href="memberlist?page=${pageVO.startPage -1}&id=${login_id}">이전 </a>
					</c:if>

					<c:forEach begin="${pageVO.startPage}" end="${pageVO.endPage}"
						var="idx">
						<c:if test="${pageVO.page == idx}"></c:if>
						<li><a href="memberlist?page=${idx}&id=${login_id}">${idx}</a></li>

					</c:forEach>
					<c:if test="${pageVO.next}">
						<a href="memberlist?page=${pageVO.endPage +1}&id=${login_id}"> 다음 </a>
					</c:if>
				</ul>
			</div>
		</div>
