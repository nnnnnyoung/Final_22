			...
			<div class="block-27">
				<ul>
					...
					<c:if test="${pageVO.prev}">
						<a href="comList?page=${pageVO.startPage -1}&id=${login_id}">이전 </a>
					</c:if>

					<c:forEach begin="${pageVO.startPage}" end="${pageVO.endPage}"
						var="idx">
						<c:if test="${pageVO.page == idx}"></c:if>
						<li><a href="comList?page=${idx}&id=${login_id}">${idx}</a></li>

					</c:forEach>
					<c:if test="${pageVO.next}">
						<a href="comList?page=${pageVO.endPage +1}&id=${login_id}"> 다음 </a>
					</c:if>
				</ul>
			</div>
		</div>
